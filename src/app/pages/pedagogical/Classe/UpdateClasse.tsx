...
import { useUpdateClasseData } from '../../../app/api/pedagogical/classe';
...
export const ClassForm = ({ classe }: any) => {
  ...
  const { put, data, loading, success, error } = useUpdateClasseData();
  ...
  const onSubmit = (form: any) => put({ ...form, ...{ id: classe?.id } })
  useEffect(() => {
    reset(classe);
  }, [classe, reset]);
  ...
  const Addon=()=><Button variant='danger'>Eliminar</Button>
  return (<>
    {success ? <MessageScreen message={"Turma registada com successo"} data={classe} status={'success'} /> : null}
    {!success ?
