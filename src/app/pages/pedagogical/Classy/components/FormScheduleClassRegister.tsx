...
export const FormScheduleClassRegister = ({ updateParams, show, item, handleClose, classy }: any) => {
    ...
    useMemo(async () => {
        const { response: { data: response, status } } = await Api.get({
            ...
            params: {
                pageSize: 100,
                'where[curricularPlanId]': classy?.course?.curricularPlanId,
                'where[semester]': classy?.semester,
            }
        });
